          ...
          <ScrollReveal animation="fadeUp">
            <div className="max-w-4xl">
              ...
              <h1 className="text-4xl md:text-6xl font-bold mb-6 leading-tight">
                Estrutura <span className="text-[var(--green)]">Organizacional</span>
              </h1>
      ...
      <div className="section-alt">
        ...
        <StaggeredReveal staggerDelay={200} className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {[
            {
              title: "Smart Cities",
              description: "Especialistas em soluções para cidades inteligentes e sustentáveis.",
              members: "8 especialistas",
              focus: ["IoT Urbano", "Big Data", "Mobilidade"],
              color: "var(--green)"
            },
            {
              title: "GovTech",
              description: "Desenvolvimento de tecnologias para modernização do setor público.",
              members: "12 desenvolvedores",
              focus: ["APIs", "Microserviços", "Integração"],
              color: "var(--primary)"
            },
            {
              title: "Dados e IA", 
              description: "Análise de dados e desenvolvimento de soluções com inteligência artificial.",
              members: "6 analistas",
              focus: ["Machine Learning", "Analytics", "Dashboards"],
              color: "var(--accent)"
            },
            {
              title: "UX/UI Design",
              description: "Design centrado no usuário para interfaces públicas eficientes.",
              members: "4 designers",
              focus: ["Research", "Protótipo", "Acessibilidade"],
              color: "var(--orange)"
            }
          ].map((nucleo, index) => (
            <div key={index} className="interactive-card bg-[var(--card)] p-6 rounded-2xl border-2 border-[var(--border)]">
              <div className="mb-4">
                <div 
                  ...
                  style={{ backgroundColor: `${nucleo.color}15`, color: nucleo.color }}
                >
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          <ScrollReveal animation="fadeRight">
            <div>
              <div className="section-title">
                <span className="bar" />
                <h2 className="text-3xl md:text-4xl font-bold text-[var(--primary)]">Comitês Estratégicos</h2>
              </div>
              <div className="mt-8 space-y-8">
                {[
                  {
                    title: "Comitê de Governança",
                    description: "Supervisiona políticas, processos e conformidade institucional.",
                    members: ["Direção", "Coordenadores", "Representantes"]
                  },
                  {
                    title: "Comitê de Inovação",
                    description: "Avalia novas tecnologias e metodologias para adoção institucional.",
                    members: ["Especialistas", "Pesquisadores", "Consultores"]
                  },
                  {
                    title: "Comitê de Qualidade",
                    description: "Garante padrões de qualidade em projetos e entregas.",
                    members: ["Analistas", "Auditores", "Gestores"]
                  }
                ].map((comite, index) => (
                  <div key={index} className="bg-[var(--card)] p-6 rounded-2xl border-2 border-[var(--border)]">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {comite.members.map((member, memberIndex) => (
                        <span 
                          key={memberIndex}
                          className="text-xs px-3 py-1 bg-[var(--accent)]/10 text-[var(--accent)] rounded-full"
                        >
                          {member}
                        </span>
                      ))}
          ...
          <ScrollReveal animation="fadeLeft">
            <div>
              <div className="section-title">
                <span className="bar" />
                <h2 className="text-3xl md:text-4xl font-bold text-[var(--primary)]">Conselho Consultivo</h2>
              </div>
              <div className="mt-8">
                <div className="bg-gradient-to-br from-[var(--accent)]/10 via-[var(--card)] to-[var(--green)]/10 p-8 rounded-2xl border-2 border-[var(--accent)]/20">
                  <div className="space-y-6">
                    {[
                      {
                        name: "Dr. João Silva",
                        role: "Especialista em Smart Cities",
                        institution: "Universidade Estadual de Maringá"
                      },
                      {
                        name: "Profa. Maria Santos",
                        role: "Pesquisadora em Governança Digital",
                      ...
                      <div key={index} className="flex items-start gap-4">
                        <div className="size-12 bg-[var(--accent)]/20 rounded-full flex items-center justify-center flex-shrink-0">
                          <span className="text-[var(--accent)] font-bold text-sm">
                            {membro.name.split(' ').map(n => n[0]).join('')}
                          </span>
                        </div>
                        <div>
                          <h4 className="font-bold text-[var(--primary)]">{membro.name}</h4>
                          <p className="text-sm text-[color:var(--muted)] mb-1">{membro.role}</p>
                          <p className="text-xs text-[var(--accent)]">{membro.institution}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="mt-6 pt-6 border-t border-[var(--border)]">
                    <p className="text-sm text-[color:var(--muted)]">
                      O Conselho Consultivo atua como órgão de assessoramento estratégico, 
                      contribuindo com expertise técnica e visão externa para o desenvolvimento institucional.
                    </p>
                  </div>
                </div>
              </div>
