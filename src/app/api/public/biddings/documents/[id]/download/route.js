...
async function incrementDownloadCountIfPossible(id) {
  try {
    const cols = await prisma.$queryRaw(
      Prisma.sql`
        SELECT column_name
        ...
        WHERE table_schema = 'public'
          AND table_name = 'bidding_documents'
          AND column_name IN ('downloadCount', 'download_count')
      `
    );
    const names = new Set((cols || []).map((c) => c.column_name));

    if (names.has('downloadCount')) {
      await prisma.$executeRaw(
        Prisma.sql`UPDATE "bidding_documents" SET "downloadCount" = COALESCE("downloadCount", 0) + 1 WHERE "id" = ${id}`
      );
    } else if (names.has('download_count')) {
      await prisma.$executeRaw(
        Prisma.sql`UPDATE "bidding_documents" SET "download_count" = COALESCE("download_count", 0) + 1 WHERE "id" = ${id}`
      );
    }
  } catch {
    // não bloquear
...
export async function GET(_request, { params }) {
  try {
    ...
    await incrementDownloadCountIfPossible(id);
    ...
    headers.set('Content-Disposition', `attachment; filename="${doc.fileName || 'documento'}"`);

    return new NextResponse(body, { status: 200, headers });
  } catch (error) {
    console.error('❌ Erro ao baixar documento (public):', error);
